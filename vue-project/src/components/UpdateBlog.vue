<template>
    ...
    <div class="flex flex-row justify-center mt-40 space-x-12" >
         <div class="w-3/12 text-xl">
             ...
             <form @submit.prevent="submitForm()" class="border border-blue-500 rounded-lg p-12 space-y-9">
                 <h1 class=" text-center text-2xl font-bold">Update Blog</h1>
             ...
             <div class=" text-center">
                 <input type="submit" value="Update" class="border border-black w-32 h-10 bg-blue-500 text-white rounded-lg" >
             </div>
             </form>
         </div>
 ...
 <script setup>
    ...
    const getPost = () => {
         axios.get(`http://127.0.0.1:8000/blogpost/blogs/${blogId.value}`,{ withCredentials: true })
        .then(response => {
            blogs.value = response.data;
            formData.value={
                title: blogs.value.title,
                content: blogs.value.Content
            }
        })
        .catch(error => {
            console.error(error);
    ...
    const submitForm = () => {
         axios.patch(`http://127.0.0.1:8000/blogpost/blogs/${blogId.value}/`,{
        ...
        .catch(error => {
            console.error(error);
            $toast.error("Blog update unsuccessful",{ position: 'bottom-right'});
        });
    };
