          ...
          <div v-for="blog in blogs">
            <div class="bg-white border border-blue-500 shadow-md rounded-lg p-6">
              ...
              <div class="flex flex-row-reverse">
                        <button class="ring-2 ring-blue-500 rounded-lg text-xl py-2 px-5" @click="removePost(blog.id)">Remove</button>
                        <button class="ring-2 ring-blue-500 rounded-lg mr-11 text-xl py-2 px-5" @click="restorePost(blog.id)">Restore</button>
                </div>
            </div>
          </div>
        </div>
...
const getPost = () => {
    return axios.get("http://127.0.0.1:8000/blogpost/blogs/?deleted=true",{ withCredentials: true })
        .then(response => {
...
const restorePost = (blogId) => {
    axios.post(`http://127.0.0.1:8000/blogpost/blogs/${blogId}/restore/`,{},{ withCredentials: true })
        .then(response => {
            $toast.success('Blog post restore successfully.');
            router.push('/bin')
        
        })
        .catch(error => {
            console.error('Error deleting user:', error);
            $toast.error("Blog post restore  unsuccessful",{ position: 'bottom-right'});
        });
    };
const removePost = (blogId) => {
    axios.delete(`http://127.0.0.1:8000/blogpost/blogs/${blogId}/hard-delete/`,{ withCredentials: true })
        .then(response => {
            $toast.success('Blog post remove successfully.');
            router.push('/bin')
        
        })
        .catch(error => {
            console.error('Error deleting user:', error);
            $toast.error("Blog post remove  unsuccessful",{ position: 'bottom-right'});
        });
    };
onMounted(getPost);
